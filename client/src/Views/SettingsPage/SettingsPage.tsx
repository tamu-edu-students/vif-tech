...
class MyEventsPage extends React.Component<Props, OwnState> {
  ...
  private _renderRoutes(): JSX.Element[] {
    ...
    // }
    return [
      // <Route exact path={this.props.parentPath+'/allowlists'} ren>
      //   <ManageAllowlists />
      // </Route>
    ]
  }
        ...
        <Switch>
          <Route exact path={parentPath} render={(routeProps: any) => (
            <div className="settings-page__links">
              <Link className="btn-wire" to={`${parentPath}/manage-allowlists`}>Manage Allowlists</Link>
            </div>
            )}
          />
          
          <Route path={parentPath+'/manage-allowlists'} render={(routeProps: any) => (
              <ManageAllowlists {...routeProps} />
            )}
          />
